...
''')
    ...
    file = red_badge
    logembed.set_thumbnail(url='attachment://image.png')
    await banlog.send(file=file,embed=logembed)
...
''')
    ...
    if timetype == 's':
        ...
        await ctx.response.send_message(embed=embed)
        await mutelog.send(file=file,embed=logembed)
    else:
        if timetype == 'm':
...
async def slash_command(ctx:discord.Interaction,id:discord.User,reason:str):
    ...
    await banlog.send(file=file,embed=embed2)
...
''')
    ...
    if allowedrole1 in ctx.user.roles or allowedrole2 in ctx.user.roles:
        await recievechannel.send(file=file,embed=logembed)
