...
botping = f'<@{botid}>'
botversion = 'CordCrafter 1.5.4 [RELEASE]'
botname = 'CordCrafter'
# Links
...
async def on_message(message:discord.Message):
    ...
    command = '/tellraw @a ["",{"text":"[@'+name+'] ","color":"blue","clickEvent":{"action":"copy_to_clipboard","value":"<@'+id+'>"},"hoverEvent":{"action":"show_text","contents":[{"text":"This message was sent from Discord using CordCrafter.","color":"dark_purple"}]}},{"text":"'+text+'","color":"gray"}]'
    if message.channel == channel:
        with mcrcon_client as mcr:
            try:
                ...
                if len(response) == 2:
                        response = '`Response length is 0, this command likely has no console output.`'
                await send_log_webhook(command=command,response=response,source=message.author)
            except:
                    embed = discord.Embed(colour=discord.Colour.red(),description='Something went wrong connecting to the RCON Client.<:warn:1249069667159638206>\nPlease report this on a [GitHub issue](<https://github.com/Tywrap-Studios/LasCordCrafter/issues>).\n-# `Error Code 500`',title='RCON: <:resources:1278835693900136532>')
