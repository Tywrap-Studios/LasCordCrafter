...
if actualram == 0 or actualram < 0:
    ...
    ram = f'{roundedram}MB'
botversion = 'CordCrafter 1.4.0 [RELEASE]'
# File Paths
...
async def slash_command(ctx:discord.Interaction):
    ...
    bot_member = ctx.guild.get_member_named('CordTester')
...
@bot.tree.command(name='godihatecups',description='Very important info!!')
async def slash_command(ctx:discord.Interaction):
    channel = ctx.channel
    spam_channel = ctx.guild.get_channel(spam)
    if channel == spam:
        ctx.response.send_message(content=f'''Sorry but like, a cup isn't even a certain thing. Like one cup can be bigger than the other and I find this, as someone who thinks baking is nice, fairly annoying as this unit quite extraordinarily fails at it's sole job of measuring specific amounts easily. The user has to know specifically which cup is referred to by context or previous knowledge. A recipe like "add 3 cups chicken broth and 2 cups rice" is exceptionally simple to misconstrue by a large amount. As far as my knowledge and looking up stuff a cup in the US is 236ml. Except when it's supposed to be used with rice[180ml], or when it's to be used with coffee[177ml]. God help you if you find the rare imperial cup[284ml].
It's lazy, weird and I don't even know how y'all manage to actually not make your bread implode on itself with this system.

(Sorry if I hurt your feelings while roasting "cups")''')
    else:
        ctx.response.send_message(content=f'Please post this in {spam_channel.mention}!',ephemeral=True)


@bot.tree.command(name='credits',description='Displays Credits for the bot.')
async def slash_command(ctx:discord.Interaction):
    description = '''<:resources:1278835693900136532> **Coding:**
...
-# MIT; Copyright 2024 Tywrap Studios. -- info.tywrap.studio@gmail.com
'''
...
**__People are busy. Attention spans are short. Time is precious.__**
