...
# Logger
log_filepath = os.getenv('LINUXLOGFILEPATH')
...
botping = f'<@{botid}>'
botversion = 'CordCrafter 1.5.0 [RELEASE]'
botname = 'CordCrafter'
# Links
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.custom,state='Waiting for CordCraft S2; DOWN',name='CustomStatus'), status=discord.Status.online)
    print(f'Set Discord Bot Status.')
    print('Starting Uptime Timer. . .')
    global startTime
    startTime = time.time()
    print('Started Timer.')
    print('Syncing and Adding onto Bot Tree. . .')
...
async def slash_command(ctx:discord.Interaction):
    ...
    if bot_status_game == 'on CordCraft Season 1':
...
Report any issues to the [GitHub Issue Tracker](<https://github.com/Tywrap-Studios/LasCordCrafter/issues>), and make it contain the debug info below.
''')
    await dm.send(embed=endbed)

async def send_webhook(url:str,content:str,title:str):
...
async def sub_command(ctx:discord.Interaction,member:discord.Member):
...
'''
    ...
    await ctx.response.send_message(embed=embed,content=f'{member.mention}')
...
<:arrow_under:1278834153655107646> This does not count for Console Verions and PE. Using PC Game Pass Bedrock should work.
'''
...
@notes.command(name='modlist',description='How to make a modlist using cmd.')
async def sub_command(ctx:discord.Interaction,member:discord.Member):
...
<:info:1278823933717512232> Why use this and not just one-by-one? See the GIF attached in the embed thumbnail! (Top-right)
'''
    embed = discord.Embed(colour=discord.Colour.blurple(),title='<:search:1279151128466165911> Binary Search:',description=description)
    embed.set_thumbnail(url='https://media.discordapp.net/attachments/1249069998148812930/1279156096103354448/binary_search.gif?ex=66d36a72&is=66d218f2&hm=b7dd480dfb0d4da00a1359d15ca47a0540a8de18bbe6ff8d8baf1bf0a5301788&=&width=480&height=319')
    await ctx.response.send_message(embed=embed,content=f'{member.mention}')
...
async def send_log_webhook(command:str,response:str,source:discord.Member):
    async with aiohttp.ClientSession() as session:
        ...
        webhook = discord.Webhook.from_url(url, session=session)
        embed = discord.Embed(colour=discord.Colour.blurple(),description=f':green_circle: The RCON `node2.centralhosting.au:24700` was sent a package.\n:package: Package: `{command}`\n:shield: Source: {source.mention}[`{source.id}`] -> [`{source.name}`].\n:information_source: Response: {response}',title='RCON TRIGGER:')
        time = datetime.now().strftime('%H:%M:%S')
        datetoday = date.today().strftime('%d/%m/%Y')
        embed.set_footer(text=f'On {datetoday} at {time}')
        await webhook.send(embed=embed)
...
async def send_rcon(command:str,ctx:discord.Interaction,adminOnly:bool):
    allowedrole1 = ctx.guild.get_role(discordadmin)
    allowedrole2 = ctx.guild.get_role(minecraftadmin)
    if adminOnly and allowedrole1 in ctx.user.roles or allowedrole2 in ctx.user.roles:
...
async def sub_command(ctx:discord.Interaction,content:str):
    command = f'/logtellraw targets @a "{content}"'
...
async def sub_command(ctx:discord.Interaction,player_user:str,duration:str,reason:Optional[str]):
    if reason == None:
        ...
        reason1 = f' {reason}'
    command = f'/ban {player_user} {duration}{reason1}'
    await send_rcon(command,ctx,True)
...
@cmd.command(name='pardon',description='Pardons the selected player.')
async def sub_command(ctx:discord.Interaction,player_user:str,reason:Optional[str]):
    if reason == None:
        reason1 = ''
    else:
...
async def sub_command(ctx:discord.Interaction):
    command = '/list'
    send_rcon(command,ctx,False)
    
@cmd.command(name='say',description='Explains how to broadcast a message to the server chat.')
async def sub_command(ctx:discord.Interaction):
    embed = discord.Embed(colour=discord.Colour.blurple(),description=f'<:info:1278823933717512232> You can add `?s` in front of your message and it will be put in the chat.',title='How to use this RCON Feature: <:resources:1278835693900136532>')
    await ctx.response.send_message(embed=embed,ephemeral=True)
    
@bot.command()
async def s(ctx:commands.Context,*args):
    text = ' '.join(args)
    command = '/logtellraw targets @a ["",{"text\":\"[@'+f'{ctx.author.display_name}] ","color\":\"blue","hoverEvent":'+'{"action\":\"show_text","contents":[{"text\":'+f'\"This message was sent from Discord using CordCrafter.","color\":\"dark_purple"'+'}]}},{"text":'+f'"{text}","color":"gray"'+'}]'
    with mcrcon_client as mcr:
        try:
            rcon_package_resp = mcr.command(command)
            response = f'`{rcon_package_resp}`'
            if len(response) == 2:
                    response = '`Response length is 0, this command likely has no console output.`'
            await send_log_webhook(command=command,response=response,source=ctx.author)
        except:
                embed = discord.Embed(colour=discord.Colour.red(),description='Something went wrong connecting to the RCON Client.<:warn:1249069667159638206>\nPlease report this on a [GitHub issue](<https://github.com/Tywrap-Studios/LasCordCrafter/issues>).\n-# `Error Code 500`',title='RCON: <:resources:1278835693900136532>')
